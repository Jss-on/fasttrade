...
int main() {
    ...
    std::cout << "=== Binance Connector Test ===" << std::endl;
    ...
    connector.on_disconnect = []() {
        std::cout << "[INFO] Binance connector disconnected" << std::endl;
    };
    ...
    if (market_tick_count > 0 && trade_tick_count > 0) {
        std::cout << "✓ Binance connector test PASSED!" << std::endl;
    } else {
        std::cout << "✗ Binance connector test FAILED - No data received" << std::endl;
        return 1;
    }
